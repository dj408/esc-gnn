...
from training.prep_dataset import get_torchmd_qm9_splits
...
def main():
    ...
    if config.dataset_config.use_torchmd_qm9_splits:
        # If we're using the TorchMD-QM9 splits, we need to get the test set
        # indices from the bespoke method in data_processing/data_utilities.py
        splits_dict = get_torchmd_qm9_splits(len(dataset))
    else:
        # Otherwise, recreate the random splits using the seed
    ...
    if key_lower in ('escgnn', 'escgnn_radial'):
        # prepare_escgnn_model internally handles both standard and radial variants
        config, model, dataloader_dict = prepare_escgnn_model(
            config, dataloader_dict, acc=acc
        )
    elif key_lower == 'visnet':
        from training.prep_model import prepare_visnet_model
        config, model, dataloader_dict = prepare_visnet_model(config, dataloader_dict, acc=acc)
    else:
