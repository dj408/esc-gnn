...
python compute_infogain_scales.py \
  ...
  python3.11 /scripts/python/compute_infogain_scales.py \
    ...
    --h5_path <WORKING_DIR>/data/ellipsoids/pq_tensor_data.h5 \
    --output_dir <WORKING_DIR>/data/ellipsoids/infogain_scales/ \
    --track vector \
    --divergence l1 \
    --device cuda

python ../scripts/python/compute_infogain_scales.py \
  --config ../config/yaml_files/borah_qm9_escgnn.yaml \
  --h5_path <WORKING_DIR>/escgnn/data/QM9/pq_tensor_data_tikhonov.h5 \
  --output_dir <WORKING_DIR>/escgnn/data/QM9/infogain_scales/ \
  --track vector \
  --divergence ot \
  --device cuda \
  --n_samples 8192 \
...
def filter_indices_in_h5(
    ...
    Args:
        dataset: ESCGNNDatasetHDF5 (or compatible) containing `data_list` with `original_idx`.
