...
--config_dir=../config/yaml_files/ellipsoids \
--dataset=ellipsoids \
--k_folds=5 \
--target=random_harmonic_normals \
--models=escgnn,escgnn_ablate_vec,legs,gcn,gat,gin,egnn \

AUTOMATED ARGUMENTS:
(1) Setting --target=diameter (default) will use the flag 
  --invariant_pred 
  for the 'escgnn' model (no ablations).
...
(2) Setting --target=random_harmonic_normals will cause the script to use 
  ...
  --target_dim=3
  For 'escgnn' model, --scalar_feat_key=x will also be used here, to prevent collision between x and pos (scalar and vector features, respectively).
END
...
# Normalize and detect whether escgnn_ablate_vec was requested
ESCGNN_ABLATE_VEC_IN_LIST=false
REQUESTED_MODELS=( )
if [[ -n "$MODELS_LIST" ]]; then
  ...
  for m in "${REQUESTED_MODELS[@]}"; do
    if [[ "$m" == "escgnn_ablate_vec" ]]; then
      ESCGNN_ABLATE_VEC_IN_LIST=true
      break
    fi
  done
fi
...
for idx in "${!MODEL_YAMLS[@]}"; do
  ...
  MODEL_EXTRA_ARGS=""
  if [[ "$MODEL_KEY" == "escgnn_ablate_vec" ]]; then
    MODEL_EXTRA_ARGS+=" --ablate_vector_track"
  fi
  if [[ "$DATASET" == "ellipsoids" && "$MODEL_KEY" == "escgnn" ]]; then
