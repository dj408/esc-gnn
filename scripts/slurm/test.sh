...
show_help() {
    ...
    echo "  sbatch $0 --config=borah_qm9_escgnn.yaml --dataset=QM9 --experiment_dir=/path/to/exp_dir --conda_env=myenv"
    echo ""
...
if [ -z "${CONFIG:-}" ]; then
    ...
    CONFIG_PATH="${ROOT_DIR}/config/yaml_files/${CONFIG}"
fi
...
accelerate launch \
  ...
  --mixed_precision=no \
  --dynamo_backend=no \
"${ROOT_DIR}/scripts/python/test_model.py" \
