...
def get_clargs():
    ...
    parser.add_argument(
        ...
        help='Filepath of yaml configuration file. If omitted when resuming'
             ' from a snapshot, the script will fall back to the original'
             ' config saved inside the experiment directory.'
    )
    # Alias for --config_path to match common usage in scripts/notebooks
    parser.add_argument(
        '--config', type=str, dest='config_path', required=False,
        help='Alias for --config_path. Path to YAML configuration file.'
    )
    ...
    parser.add_argument(
        ...
        help='Ablate vector track of ESCGNN model'
    )
    parser.add_argument(
        '--ablate_scalar_track', action='store_true',
        help='Ablate scalar track of ESCGNN model'
    )
    parser.add_argument(
